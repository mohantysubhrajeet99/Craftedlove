<template>
  <div>
    ...
    <!-- FLOATING DEMO SHORTCUT / SYSTEM ROLES TRIGGER -->
    <div class="fixed bottom-4 left-4 z-40">
      <button 
        @click="demoMenuOpen = !demoMenuOpen"
        class="w-12 h-12 rounded-full bg-stone-900 text-white hover:bg-stone-850 flex items-center justify-center shadow-lg hover:shadow-xl active:scale-95 transition-all outline-none"
        aria-label="Demo Role Toggle"
      >
        <!-- Settings gear icon -->
        <svg class="w-6 h-6 animate-[spin_10s_linear_infinite]" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.43l-1.003.828c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.43l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 0 1 0-.255c.007-.378-.138-.75-.43-.991l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.28z"/><circle cx="12" cy="12" r="3"/></svg>
      </button>
      
      <!-- Demo Options Dropdown -->
      <div v-show="demoMenuOpen" class="absolute left-0 bottom-14 w-60 bg-white border border-stone-100 rounded-2xl shadow-xl p-3 z-50 text-xs text-stone-700 space-y-2.5 animate-fadeIn">
        <p class="font-bold text-stone-855 border-b pb-1">Demo Quick Actions</p>
        <div class="flex flex-col gap-1.5">
          <button @click="setDemoRole('customer')" class="w-full text-left p-2 rounded-xl hover:bg-rose-50 hover:text-rose-600 transition-colors font-medium">Log In: Demo Customer</button>
          <button v-if="store.currentUser" @click="setDemoRole('logout')" class="w-full text-left p-2 rounded-xl hover:bg-stone-50 text-rose-500 transition-colors font-medium">Log Out current session</button>
          <div class="border-t border-stone-100/60 my-0.5"></div>
          <button @click="setDemoRole('reset')" class="w-full text-left p-2 rounded-xl hover:bg-red-50 text-red-650 transition-colors font-medium flex items-center justify-between">
            Reset Demo Database
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99"/></svg>
          </button>
        </div>
        <div class="text-[10px] text-stone-400 bg-stone-50 p-2 rounded-lg leading-relaxed">
          Click these triggers to immediately verify role changes and live order timelines in real-time.
        </div>
      </div>
    </div>
